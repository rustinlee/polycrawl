...
exports.simulateCombat = function(aggressor, target, level, aggressorSocketID, targetSocketID) {
	...
	if (attackHit) {
		...
		if (target.HP <= 0) {
			level.gameEntities = _und.reject(level.gameEntities, function (mob) {
				return mob.id === target.id;
			});

			if (targetSocket) { //respawn mob if a player is controlling it
				targetSocket.emit('chatMessage', { message: 'You have died!' });
				var map = getMap(targetSocket.game_player.mapIndex);
				targetSocket.game_player = new Mob(mobDefinitions['human'], map.playerSpawn.x, map.playerSpawn.y, targetSocket.color, targetSocket.id);
